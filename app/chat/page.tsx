...
function ChatContent() {
  ...
  // ✅ FIX 1: Ref se latest userId aur activeKey access karo — stale closure se bachao
  const userIdRef = useRef(userId);
  const activeKeyRef = useRef(activeKey);
  useEffect(() => { userIdRef.current = userId; }, [userId]);
  useEffect(() => { activeKeyRef.current = activeKey; }, [activeKey]);

  // ✅ FIX 2: Table naam "messages" kiya, function naam bhi fix
  const saveMessage = useCallback(async (role: string, content: string) => {
    const uid = userIdRef.current;
    const mid = activeKeyRef.current;
    if (!uid) return;
    const { error } = await supabase.from("messages").insert([
      { user_id: uid, role, content, model_id: mid }
    ]);
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      else console.warn("User not logged in — messages won't be saved");
    });
  }, []);
  ...
  const sendMessage = useCallback(async () => {
    ...
    await saveMessage("user", cur); // ✅ FIX 3: userId check saveMessage ke andar hai
    setMessage("");
    setLoading(true);
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          message: cur,
          userId: userIdRef.current || "demo-user",
          history: messages.slice(-10), // ✅ FIX 4: 5 se 10 kiya — better memory
          modelId: activeKeyRef.current
        }),
      });
    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @keyframes dynamicGlowPulse { 
          0%, 100% { box-shadow: 0 0 20px var(--glow-low), 0 40px 100px rgba(0,0,0,.9); border-color: var(--border-low); } 
          50% { box-shadow: 0 0 45px var(--glow-high), 0 40px 100px rgba(0,0,0,.9); border-color: var(--border-high); } 
        }
        .luna-shell-glow { animation: dynamicGlowPulse 5s ease-in-out infinite; transition: all 0.5s ease-in-out; }
      ...
      <main style={{ height:"100vh", overflow:"hidden", display:"flex", alignItems:"center", justifyContent:"center", background:"#07040f", position:"relative", zIndex:10 }}>
        <div
          ...
          style={{
            width:"660px", maxWidth:"97vw", height:"min(92vh,780px)", display:"flex", flexDirection:"column",
            background:"rgba(10,6,20,.85)", borderRadius:"32px", border:"1px solid", overflow:"hidden",
            backdropFilter:"blur(60px) saturate(180%)", position:"relative",
        ...
        >
          ...
          <div style={{ padding:"18px 26px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0, position:"relative" }}>
            ...
            <Link href="/" style={{ display:"flex", alignItems:"center", gap:"14px", textDecoration:"none" }}>
              <div style={{ width:"40px", height:"40px", borderRadius:"50%", background:`linear-gradient(135deg, ${character?.color || 'rgba(220,80,160,.35)'}, rgba(140,60,220,.28))`, display:"flex", alignItems:"center", justifyContent:"center" }}>
                <div style={{ width:"9px", height:"9px", borderRadius:"50%", background:"white", boxShadow:`0 0 14px ${character?.color || 'magenta'}` }} />
              </div>
          ...
          <div className="luna-scroll" style={{ flex:1, overflowY:"auto", padding:"22px 22px 10px", display:"flex", flexDirection:"column", gap:"18px" }}>
            ...
            {messages.map((msg, i) => (
              <div key={i} style={{ display:"flex", flexDirection:"column", alignItems: msg.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{
                  maxWidth: msg.role === "user" ? "65%" : "80%", padding:"12px 16px", fontSize:"14px", color:"white",
          ...
          <div style={{ padding:"13px 18px 20px", flexShrink:0 }}>
            <div className="luna-pill" style={{ display:"flex", alignItems:"center", gap:"10px", background:"rgba(255,255,255,.04)", border:`1px solid ${character?.color ? character.color.replace(',1)', ',0.25)') : 'rgba(255,120,170,.25)'}`, borderRadius:"100px", padding:"8px 20px" }}>
              ...
              <button onClick={sendMessage} disabled={!message.trim() || loading}
                style={{ background: character?.color || "magenta", color:"black", fontWeight:600, border:"none", borderRadius:"20px", padding:"6px 16px", cursor:"pointer", transition:"all 0.3s" }}>
                Send
              </button>
            </div>
